...
function hasCollidedWithApple() {
  // Check if head's row matches apple's row AND head's column matches apple's column
...
function hasCollidedWithSnake() {
  ...
  */
  ...
  for (var i = 1; i < snake.body.length; i++) {
    ...
    // 2. Compare the head's row and column with this body part's row and column
    if (snake.head.row === part.row && snake.head.column === part.column) {
      // If they match, the snake has crashed into itself!
      return true;
    }
  }
